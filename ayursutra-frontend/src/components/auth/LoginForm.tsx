...
import { exchangeUidForJwt, selectUserRole, checkUserExists } from "@/lib/api";
import {
  signInWithPopup,
...
export default function LoginForm({ onClose }: LoginFormProps) {
  ...
  async function handleRoleSelected(role: string) {
    if (!pendingUser) return;

    setIsLoading(true);
    try {
      // Try to save role to backend
      const { token } = await selectUserRole(pendingUser.uid, role, {
  ...
  async function handleGoogle() {
    if (!firebaseAuth || !firebaseGoogleProvider) {
      alert(
        "Firebase authentication is not configured. Please use email/password for now."
      );
      return;
    }
    ...
    try {
      ...
      const res = await signInWithPopup(firebaseAuth, firebaseGoogleProvider);
      ...
      try {
        ...
        if (userExists) {
          // Existing user - get their role and redirect to dashboard
          const { token, role } = await exchangeUidForJwt(idToken, {
            uid: res.user.uid,
    ...
    try {
      const fn =
        mode === "login"
          ? signInWithEmailAndPassword
          : createUserWithEmailAndPassword;
      const res = await fn(firebaseAuth, email, password);
      const idToken = await res.user.getIdToken();
      ...
      if (error.code === "auth/user-not-found") {
        ...
        alert(
          `${mode === "login" ? "Login" : "Sign up"} failed. Please try again.`
        );
      }
    } finally {
      setIsLoading(false);
  ...
  if (showRoleSelection && pendingUser) {
    return (
      <RoleSelection user={pendingUser} onRoleSelected={handleRoleSelected} />
    );
  }

  return (
    <div className="fixed inset-0 z-[100] grid place-items-center p-4 bg-black/50">
      <motion.div
