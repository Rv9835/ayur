...
router.get("/patient/:patientId", async (req, res) => {
    try {
        ...
        const uid = req.user?.uid || patientId;
        ...
        if (typeof patientId === "string" && !patientId.includes(".")) {
            patientUser = await User_1.User.findOne({ uid: patientId });
            if (!patientUser) {
                patientUser = await User_1.User.findById(patientId);
            }
        }
        else {
            patientUser = await User_1.User.findById(patientId);
        ...
        if (!patientUser && role === "patient" && uid === patientId) {
            patientUser = await User_1.User.create({
                uid: patientId,
                name: "Demo Patient",
        ...
        console.log("Permission check (dev):", {
            ...
            uid,
            patientId,
            patientUserUid: patientUser.uid,
        });
        // Get all appointments for the patient
        const appointments = await Appointment_1.Appointment.find({ patient: patientUser._id })
        ...
        const averageRating = appointments.reduce((sum, apt) => sum + (apt.rating || 0), 0) /
            totalSessions || 0;
        ...
        const therapyDistribution = appointments.reduce((acc, apt) => {
            const therapyName = apt.therapy?.name || "Unknown";
        ...
        monthlyData.forEach((month) => {
            month.rating = month.totalRating / month.ratingCount;
        });
        ...
        const progressData = {
            metrics: {
                ...
                currentStreak,
                longestStreak,
                improvementRate: totalSessions > 0
            ...
            recentSessions: appointments
                ...
                id: apt._id,
                date: apt.startTime,
                therapy: apt.therapy?.name || "Unknown",
                therapist: apt.doctor?.name || "Unknown",
...
router.patch("/session/:sessionId/feedback", async (req, res) => {
    try {
        const { sessionId } = req.params;
        const { rating, notes, painLevel, energyLevel, moodLevel, sleepQuality, overallWellness, } = req.body;
        // Dev: allow unauthenticated access for feedback update
        const appointment = await Appointment_1.Appointment.findById(sessionId);
        if (!appointment) {
        ...
        if (painLevel !== undefined)
            appointment.painLevel = painLevel;
        if (energyLevel !== undefined)
            appointment.energyLevel = energyLevel;
        if (moodLevel !== undefined)
            appointment.moodLevel = moodLevel;
        if (sleepQuality !== undefined)
            appointment.sleepQuality = sleepQuality;
...
router.get("/patient/:patientId/goals", async (req, res) => {
    try {
        const { patientId } = req.params;
        const uid = req.user?.uid || patientId;
        const role = req.user?.role || "patient";
        ...
        const goals = [
            {
                ...
                description: "Achieve 50% reduction in back pain through regular therapy sessions",
            ...
            },
            {
                ...
                description: "Maintain consistent 8-hour sleep schedule with better quality",
...
router.get("/patient/:patientId/achievements", async (req, res) => {
    try {
        const { patientId } = req.params;
        const uid = req.user?.uid || patientId;
        const role = req.user?.role || "patient";
